  ...
  _createClass(Migrator, [{
    ...
    value: function latest(config) {
      ...
      return this._migrationData().tap(validateMigrationList).spread(function (all, completed) {
        return _this._runBatch((0, _lodash.difference)(all, completed), 'up');
      });
    }
    ...
    value: function currentVersion(config) {
      ...
      return this._listCompleted(config).then(function (completed) {
        var val = (0, _lodash.chain)(completed).map(function (value) {
    ...
    value: function _listAll(config) {
      this.config = this.setConfig(config);
      return Promise.promisify(fs.readdir, { context: fs })(this._absoluteConfigDir()).then(function (migrations) {
        return (0, _lodash.filter)(migrations, function (value) {
          var extension = path.extname(value);
          return (0, _lodash.includes)(['.co', '.coffee', '.eg', '.iced', '.js', '.litcoffee', '.ls'], extension);
        }).sort();
      });
    }
    ...
    value: function _ensureFolder() {
      ...
      return Promise.promisify(fs.stat, { context: fs })(dir)['catch'](function () {
    ...
    value: function _runBatch(migrations, direction) {
      ...
      return this._getLock().then(function () {
        return Promise.all((0, _lodash.map)(migrations, (0, _lodash.bind)(_this7._validateMigrationStructure, _this7)));
      }).then(function () {
        ...
        if (error instanceof LockError) {
          ...
          helpers.warn('If you are sure migrations are not running you can release ' + 'lock manually by deleting all the rows from migrations lock table: ' + _this7._getLockTableName());
        } else {
    ...
    value: function _generateStubTemplate() {
      ...
      return Promise.promisify(fs.readFile, { context: fs })(stubPath).then(function (stub) {
        return (0, _lodash.template)(stub.toString(), null, { variable: 'd' });
      });
    }
    ...
    value: function _writeNewMigration(name, tmpl) {
      ...
      return Promise.promisify(fs.writeFile, { context: fs })(path.join(dir, filename), tmpl(config.variables || {}))['return'](path.join(dir, filename));
    }
